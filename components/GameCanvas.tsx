...
import { CAN_IMAGE_URLS, GAME_WORLD_SIZE, CRITTER_ATLAS, CRITTER_FPS_IDLE, CRITTER_FPS_WALK, QUEBEC_BORDER_Y, SPEED_BOOST_DURATION, ROADS, ROAD_TEXTURE_URL, ROAD_TILE_SIZE, isPointInWater } from '../constants.ts';
...
type GroundDecal = { position: Vector2; radius: number; rotation: number; sides: number; color: string; alpha: number; };
...
const createGroundDecals = (): GroundDecal[] => {
  const rng = makeMulberry32(2024);
  const palette = ['#87b16a', '#6f9b59', '#a4c47a', '#c5d38a'];
  const decals: GroundDecal[] = [];
  for (let i = 0; i < 120; i++) {
    const pos = { x: rng() * GAME_WORLD_SIZE.width, y: rng() * GAME_WORLD_SIZE.height };
    if (isPointInWater(pos)) continue;
    decals.push({
      position: pos,
      radius: 32 + rng() * 48,
...
const GameCanvas: React.FC<GameCanvasProps> = ({ gameStateRef, onSetTargetPosition }) => {
  ...
  const [groundDecals] = useState<GroundDecal[]>(() => createGroundDecals());
  const [loadingProgress, setLoadingProgress] = useState({ loaded: 0, total: 1 });
  const [allAssetsLoaded, setAllAssetsLoaded] = useState(false);

  useEffect(() => {
    let aborted = false;
    const imagesToLoad = Array.from(new Set([...CAN_IMAGE_URLS, CRITTER_ATLAS.image, ROAD_TEXTURE_URL]));
    let loadedCount = 0;
    ...
    const loadImage = async (url: string) => {
      try {
        ...
        if (url === ROAD_TEXTURE_URL) {
          roadPatternCanvasRef.current = null;
        ...
        if (!aborted) {
          ...
          if (loadedCount === imagesToLoad.length) {
            setTimeout(() => !aborted && setAllAssetsLoaded(true), 500);
          }
        }
      }
    };
  ...
  useEffect(() => {
    ...
    const ensureGroundPattern = () => {
      if (!patternCanvasRef.current) {
        ...
        if (pctx) {
          ...
          pctx.fillStyle = 'rgba(112, 150, 110, 0.25)';
          for (let i = 0; i < 22; i++) {
          ...
          pctx.fillStyle = 'rgba(245, 231, 176, 0.15)';
          for (let i = 0; i < 14; i++) {
            pctx.save();
            pctx.translate(Math.random() * 256, Math.random() * 256);
            pctx.rotate(Math.random() * Math.PI * 2);
            pctx.beginPath();
            pctx.moveTo(-6, 0);
            pctx.lineTo(0, -10);
    ...
    const ensureRoadPattern = () => {
      ...
      if (!roadPatternRef.current) {
        roadPatternRef.current = ctx.createPattern(roadPatternCanvasRef.current, 'repeat');
      }
    ...
    const render = (time: number) => {
      ...
      groundDecals.forEach(decal => {
        if (decal.position.x < viewBounds.left - decal.radius ||
            decal.position.x > viewBounds.right + decal.radius ||
            decal.position.y < viewBounds.top - decal.radius ||
            decal.position.y > viewBounds.bottom + decal.radius) {
          return;
        }
        const screen = worldToScreen(decal.position, camera, canvasSize);
        ctx.save();
        ctx.translate(screen.x, screen.y);
        ctx.rotate(decal.rotation);
        ctx.beginPath();
        for (let i = 0; i < decal.sides; i++) {
          const theta = (i / decal.sides) * Math.PI * 2;
          const radius = decal.radius * (0.6 + Math.sin(theta * 3 + decal.rotation) * 0.1);
          const px = Math.cos(theta) * radius;
          const py = Math.sin(theta) * radius * 0.6;
      ...
      ROADS.forEach(road => {
          ...
          if (maxX < viewBounds.left || minX > viewBounds.right || maxY < viewBounds.top || minY > viewBounds.bottom) {
            return;
          }
      ...
      bridges.forEach(b => {
          ...
          if (roadPattern) {
            ...
            ctx.fillStyle = b.repairGag ? 'rgba(80, 60, 60, 0.85)' : 'rgba(92, 92, 92, 0.85)';
            ctx.fillRect(x, y, width, height);
          }
          ctx.strokeStyle = 'rgba(34,34,34,0.9)';
          ctx.lineWidth = 2;
          ctx.strokeRect(x, y, width, height);
          ctx.fillStyle = b.repairGag ? 'rgba(142,72,72,0.25)' : 'rgba(255,255,255,0.08)';
          ctx.fillRect(x, y, width, height);
          ctx.restore();
      });
